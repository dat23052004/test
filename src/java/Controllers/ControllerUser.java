...
public class ControllerUser extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Users p = new Users(x.getId(), name, birthday, email, phone);
            da.updateByUser(p);
            session.setAttribute("user", da.getinforbyId(p.getId()));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (oldpass.equals(current.getPassword())) {
            if (newPass.equals(cfPass)) {
                userDa.updatePassById(current.getId(), newPass);
                response.sendRedirect("Reset?mode=1");
            } else {
        ...
        if (!error.isEmpty()) {
            ...
            request.getRequestDispatcher("profile_password.jsp").forward(request, response);
        }
    }
